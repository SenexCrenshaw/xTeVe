    ...
    switch (dataType) {
        ...
        case "mapping":
            ...
            if (SERVER['settings']['enableTapiosinnTVLogos']) {
                ...
                const currentLogoUrl = data[dbKey];
...
class TVLogosFile {
     ...
     */
    newTvLogosIdPicker(currentLogoUrl) {
        ...
        const logos = SERVER['tvlogos']['files'];
        if (logos) {
            const folders = getOwnObjProps(logos);
            folders.forEach((folder) => {
                logos[folder].forEach((logo) => {
                    const option = document.createElement('option');
...
function previewChannelLogo(url) {
